...
import { Cmd, just, Maybe, nothing } from 'tea-cup-core';
import {
  item,
  Menu,
  menu,
...
export type FormMenuModel = TPModel<MenuAction>;
export type FormMenuMsg = TPMsg<MenuAction>;

export interface HasMenu {
...
export function openMenu<T extends HasMenu, M>(
  ...
  refBox: Box,
  toMsg: (msg: FormMenuMsg) => M,
): [T, Cmd<M>] {
  const mac: [FormMenuModel, Cmd<FormMenuMsg>] = open(menu, refBox);
  const newT: T = {
    ...t,
